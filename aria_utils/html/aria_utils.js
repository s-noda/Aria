...
function _play(tableID, i) {
    ...
    setTimeout("_play('" + tableID + "'," + ++i + ")", time_dat*1000);
}

function play_random(tableID){
    var table = document.getElementById(tableID);
    var rowCount = table.rows.length;
    var i = Math.floor(Math.random()*table.rows.length);
    if ( i === rowCount ) return ;
    // for (var i=1; i< rowCount; i++) {
    if (table.rows[i].cells[2].childNodes[0].value == "on") {
        motionData[i-1][motionData[i-1].length-1] = parseFloat(table.rows[i].cells[4].childNodes[0].value);
    ...
    setTimeout("play_random('" + tableID + "')", time_dat*1000);
}
